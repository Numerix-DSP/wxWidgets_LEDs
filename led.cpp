...

{
	if ( LEDBitmap->IsOk() )
	{
		...
		dc.Blit(0, 0, LEDBitmap->GetWidth(), LEDBitmap->GetHeight(), & memDC,
				0, 0, wxCOPY, TRUE);
...
{
//	wxMessageBox (_T("Got here"), _T("MyFrame::Flash"), wxOK | wxICON_INFORMATION, this);
...
{
//	wxMessageBox (_T("Got here"), _T("MyFrame::OnTimer"), wxOK | wxICON_INFORMATION, this);
...
{
	Size = size;
    SetSize (size);			// Set the size of the canvas because it is not done by parent unless it is a frame
    m_owner = parent;
	m_LED = pLED;
}
